(function () {
  ...
  const CATALOG = window.TAMA_CATALOG;
  if (!CATALOG) return;

  const closeMenus = () => root.querySelectorAll(".ms.open").forEach((w) => w.classList.remove("open"));
  ...
  const options = (opts) =>
    opts.map((o) => (Array.isArray(o) ? o : [o, o]));
  ...
  function singleSelect(optsFn, get, set) {
    ...
    const wrap = el("div", { class: "ms" }, [trigger, el("div", { class: "ms-panel" }, [list])]);

    const build = () => {
      list.innerHTML = "";
      const cur = get();
      let curTxt = "";
      options(optsFn()).forEach(([val, txt]) => {
        if (val === cur) curTxt = txt;
        const opt = el("button", { class: "ms-opt", type: "button" }, [el("span", { class: "ms-check" }), el("span", { class: "ms-opt-l", text: val })]);
    ...
    trigger.addEventListener("click", (e) => {
      ...
      closeMenus();
      if (!open) { build(); wrap.classList.add("open"); }
    });
  ...
  function boundSelect(obj, key, opts) {
    return singleSelect(() => opts, () => obj[key], (v) => { obj[key] = v; });
  }
  ...
  function transportsField() {
    ...
    LINKS.forEach(([link, desc]) => {
      ...
      if (protos.length > 1) {
        kids.push(singleSelect(() => protos, () => state.linkProto[link], (v) => { state.linkProto[link] = v; }));
  ...
  function selectField(label, opts, key, req) {
    const ss = singleSelect(opts, () => state[key], (v) => { state[key] = v; });
  ...
  function multiselect(items, arrKey, onChange) {
    const norm = options(items);
    const wrap = el("div", { class: "ms" });
    const label = el("span", { class: "ms-label" });
    const trigger = el("button", { class: "ms-trigger", type: "button" }, [label, el("span", { class: "ms-caret" })]);
    const list = el("div", { class: "ms-list" });
    const panel = el("div", { class: "ms-panel" }, [list]);
    wrap.appendChild(trigger);
    wrap.appendChild(panel);

    const summary = () => {
      const picked = norm.map((o) => o[0]).filter((v) => state[arrKey].includes(v));
    ...
    norm.forEach(([val, desc]) => {
      const opt = el("button", { class: "ms-opt", type: "button" }, [el("span", { class: "ms-check" }), el("span", { class: "ms-opt-l", text: val })]);
      if (desc && desc !== val) opt.appendChild(el("small", { class: "ms-opt-d", text: desc }));
      const sync = () => opt.classList.toggle("on", state[arrKey].includes(val));
      sync();
      opt.addEventListener("click", (e) => {
        e.preventDefault();
  ...
  function sourceCard(src, onRemove) {
    ...
    const typeSel = boundSelect(src, "type", SOURCE_TYPES);
    ...
    typeSel.addEventListener("change", () => { drawMetrics(); rebuild(); render(); });
    rebuild();
    return card;
  }
  ...
  if (flashRoot) Promise.all([
    ...
    boardSelect.addEventListener("change", () => { state.board = boardSelect.value; flashSync(); });
    ...
    const DIALOG_CSS =
      ...
      "ew-text-button:last-of-type{--md-sys-color-primary:#fff;background:var(--text);border-color:var(--text)}";
    const brandDialog = (n) => {
