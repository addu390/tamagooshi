...
function scanRow(device) {
  const row = el("li");
  row.append(el("span", "grow", device.name));
  row.append(el("span", "dim", device.address));
...
export function connectionCard(conn) {
  ...
  if (conn.locked) {
    info.append(el("div", "set-desc",
                   "Fixed by TAMA_TRANSPORT on the host, so Bluetooth pairing is off here."));

    const scan = el("button", "btn", "Scan for devices");
    scan.disabled = true;
    scan.title = "Not available in this deployment";
    buttons.append(scan);
  } else {
    if (conn.saved) {
    ...
    const scan = el("button", "btn",
                    conn.link === "ble" ? "Scan for devices" : "Connect via Bluetooth");
