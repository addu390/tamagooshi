...
void hints(Gfx& g, const char* a, const char* b) {
  const int y = g.h() - 20;
  g.c().drawFastHLine(0, y - 3, g.w(), theme::kDimmer);
  if (a) {
    std::string la = std::string("A ") + a;
    g.str(la.c_str(), 4, y, theme::kDim, typeface::body(), textdatum_t::top_left);
  }
  if (b) {
    std::string lb = std::string("B ") + b;
    g.str(lb.c_str(), g.w() - 4, y, theme::kDim, typeface::body(), textdatum_t::top_right);
  }
}

void sectionLabel(Gfx& g, const char* text) {
  g.str(text, g.w() / 2, 26, theme::kHi, typeface::body(), textdatum_t::top_center);
}
...
void listRow(Gfx& g, const Rect& r, const char* label, const char* value, bool selected,
             const lgfx::IFont* font, bool enabled) {
  SelectStyle style;
  style.radius = 4;
  style.outline = 0;
  if (!enabled) style.fill = theme::kDim;
  const uint16_t val = selectionBox(g, r, selected, style);
  const uint16_t lab = selected ? theme::kBg : (enabled ? theme::kDim : theme::kDimmer);
  const uint16_t rv = selected ? val : (enabled ? val : theme::kDimmer);

  g.str(label, r.x + 8, r.y + 3, lab, font, textdatum_t::top_left);
  if (value && value[0]) g.str(value, r.x + r.w - 8, r.y + 3, rv, font, textdatum_t::top_right);
}
...
void listView(Gfx& g, const Layout& L, const ListItem* items, int count, int sel) {
  ...
  if (L.landscape) {
    ...
    for (int i = 0; i < count; ++i) {
      ...
      listRow(g, {x, y - 2, colW, rowH - 4}, items[i].label, items[i].value, i == sel, font,
              items[i].enabled);
    }
  } else {
    ...
    for (int i = first; i < count && i - first < visible; ++i) {
      listRow(g, {4, top + (i - first) * rowH - 2, rowW, rowH - 2}, items[i].label, items[i].value,
              i == sel, font, items[i].enabled);
    }
  }
}
