...
void PromptOverlay::drawContent(Gfx& g, ShellContext& ctx, int hdrH, bool landscape) {
  ...
  g.str(page_.title.c_str(), 6, titleY, theme::kHi, typeface::body(), textdatum_t::top_left);
  if (!page_.body.empty()) {
    g.str(page_.body.c_str(), 6, bodyY, theme::kDim, typeface::body(), textdatum_t::top_left);
  }
  ...
  if (landscape) {
    ...
    ctx.character->draw(g, g.w() / 2, g.h() / 2 + 6, 44, MascotState{Expr::Alert}, now_);
  }
}
