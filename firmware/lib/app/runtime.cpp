...
void Runtime::bind(const SourceBinding& binding) {
  source_.bind(binding);
...
void Runtime::begin() {
  ...
  auto wire = [this](SourceChannel& ch, bool isHub) {
    if (!ch.valid()) return;
    ...
    ch.connection->onConnection([this, pipeline, isHub](bool connected) {
      ...
      if (isHub)
        ...
        state_.claude_connected = connected;
      state_.connected = state_.hub_connected || state_.claude_connected;
      nav_.markDirty();
      renderIfNeeded();
    });
  };
  wire(source_.hub(), true);
  wire(source_.claude(), false);
  ...
  sensor_.begin();
  gfx_.begin();
  if (source_.hub().valid()) source_.hub().connection->begin();
  if (source_.claude().valid()) source_.claude().connection->begin();
...
void Runtime::loop(uint32_t nowMs) {
  if (source_.hub().valid()) {
    source_.hub().connection->loop();
    source_.hub().pipeline->tick(nowMs);
  }
  if (source_.claude().valid()) {
    source_.claude().connection->loop();
    source_.claude().pipeline->tick(nowMs);
  }
  mapper_.poll();
  sensor_.poll();
  syncPrompt();
...
void Runtime::onGesture(const GestureEvent& event) {
  if (source_.hub().valid()) source_.hub().pipeline->onGesture(event);
  if (source_.claude().valid()) source_.claude().pipeline->onGesture(event);
}

void Runtime::onMotion(const MotionEvent& event) {
  if (source_.hub().valid()) source_.hub().pipeline->onMotion(event);
  if (source_.claude().valid()) source_.claude().pipeline->onMotion(event);
}
